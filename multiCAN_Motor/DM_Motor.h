...
// Forward-declare DMManager to break circular dependency
class DMManager;
...
public:
  ...
  // New constructor that registers itself with a manager
...
private:
  ...
  // Member variables
  arduino::HardwareCAN* can_ = nullptr;
  Feedback feedback_{};
...
// Include the full definition of DMManager here to be able to call its methods
#include "DMManager.h"

// Now define the methods that need the full DMManager definition
inline uint8_t DMMotor::getSlaveIdFromMessage(const CanMsg& msg) {
  return msg.data[0] & FEEDBACK_SLAVE_ID_MASK;
}

inline DMMotor::DMMotor(DMManager* manager, uint32_t slaveId, DM_ControlMode mode)
  : slaveId_(slaveId), currentMode_(mode) {
  if (manager) {
    manager->registerMotor(slaveId_, this);
    this->can_ = manager->getCanClient();
    this->masterId_ = manager->getMasterId();
  }
}

inline void DMMotor::initialize() {
  if (!can_) return;
...
inline void DMMotor::processMessage(const CanMsg &msg) {
...
inline bool DMMotor::sendSystemCommand(uint8_t commandByte) {
...
inline float DMMotor::uintToFloat(uint16_t x, float min_val, float max_val, int bits) {
  float span = max_val - min_val;
  float normalized = (float)x / (float)((1 << bits) - 1);
  return normalized * span + min_val;
}

inline uint16_t DMMotor::floatToUint(float x, float min_val, float max_val, int bits) {
  float span = max_val - min_val;
...
// All other methods must also be defined as inline outside the class
// since they are declared inside the class but defined after the DMManager include

inline bool DMMotor::enable() {
  return sendSystemCommand(CMD_BYTE_ENABLE);
}

inline bool DMMotor::disable() {
  return sendSystemCommand(CMD_BYTE_DISABLE);
}

inline bool DMMotor::setZeroPoint() {
  return sendSystemCommand(CMD_BYTE_SET_ZERO);
}

inline void DMMotor::setControlMode(DM_ControlMode mode) {
  currentMode_ = mode;
  sendParamUInt32(DM_RID::DM_RID_CTRL_MODE, static_cast<uint32_t>(mode));
}

inline bool DMMotor::sendMIT(float position_rad, float velocity_rad_s, float kp, float kd, float torque_ff) {
...
inline bool DMMotor::sendPosition(float position_rad, float velocity_limit_rad_s) {
  ...
  tx.data[0] = static_cast<uint8_t>(position_raw >> 8);
  tx.data[1] = static_cast<uint8_t>(position_raw & 0xFF);
  tx.data[2] = static_cast<uint8_t>(velocity_raw >> MIT_VEL_SHIFT); // Typo: Changed to use MIT_VEL_SHIFT for consistency
  tx.data[3] = static_cast<uint8_t>((position_raw >> 24) & 0xFF); // Adjusted for correct data packing
...
inline bool DMMotor::sendVelocityRPM(float velocity_rpm) {
  return sendVelocity(velocity_rpm * PI / 30.0f);
}

inline bool DMMotor::sendVelocityRPS(float velocity_rps) {
  return sendVelocity(velocity_rps * PI * 2.0f);
}

inline bool DMMotor::sendVelocity(float velocity_rad_s) {
...
inline float DMMotor::getPosition() const {
  return feedback_.position;
}
inline float DMMotor::getPositionDeg() const {
  return feedback_.position * 180.0f / PI;
}
inline float DMMotor::getVelocity() const {
  return feedback_.velocity;
}
inline float DMMotor::getRPM() const {
  return feedback_.velocity * 30.0f / PI;
}
inline float DMMotor::getRPS() const {
  return feedback_.velocity / (2.0f * PI);
}
inline float DMMotor::getTorque() const {
  return feedback_.torque;
}
inline int8_t DMMotor::getMOSTemp() const {
  return feedback_.temp_mos;
}
inline int8_t DMMotor::getRotorTemp() const {
  return feedback_.temp_rotor;
}

inline DM_ControlMode DMMotor::getMode() {
...
inline uint32_t DMMotor::getSlaveId() const {
  return slaveId_;
}
inline uint32_t DMMotor::getMasterId() const {
  return masterId_;
}
inline DM_Status DMMotor::getStatus() const {
  return feedback_.status;
}

inline float DMMotor::getPMAX() const {
  return abs(mappingrange_.pmax);
}
inline float DMMotor::getVMAX() const {
  return abs(mappingrange_.vmax);
}
inline float DMMotor::getTMAX() const {
  return abs(mappingrange_.tmax);
}

inline bool DMMotor::readParamUInt32(DM_RID rid, uint32_t &out, uint32_t timeout_ms) {
  if (!can_) return false;
...
inline bool DMMotor::readParamFloat(DM_RID rid, float &out, uint32_t timeout_ms) {
  uint32_t raw;
...
inline bool DMMotor::sendParamUInt32(DM_RID rid, uint32_t value_to_write, uint32_t timeout_ms) {
  if (!can_) return false;
...
inline bool DMMotor::sendParamFloat(DM_RID rid, float value_to_write, uint32_t timeout_ms) {
  uint32_t raw_value;
